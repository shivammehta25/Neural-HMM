...
from src.utilities.functions import logsumexp, log_clamped
...
class TransitionModel(nn.Module):
    ...
    def forward(self, log_alpha_scaled, transition_vector, state_lengths):
        ...
        not_state_lengths_mask = ~(torch.arange(T_max, out=mask_tensor).expand(
            len(state_lengths), T_max) < (state_lengths).unsqueeze(1))
