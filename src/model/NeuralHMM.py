...
class NeuralHMM(nn.Module):
    def __init__(self, hparams):
        ...
        if hparams.warm_start or hparams.checkpoint_path:
            ...
            std = sqrt(2.0 / (hparams.n_symbols +
    ...
    def re_init_embeddings(self, hparams):
        self.embedding = nn.Embedding(hparams)
    ...
    def inference(self, text_inputs):
        ...
        Returns:
            mel_outputs (list): list of len of the output of mel spectrogram each containing n_mel_channels channels
                shape: (len, n_mel_channels)
            states_travelled (list): list of phoneme travelled at each time step t
                shape: (len)
        """

        text_lengths = text_inputs.new_tensor(
            text_inputs.shape[1]).unsqueeze(0)
        embedded_inputs = self.embedding(text_inputs).transpose(1, 2)
        encoder_outputs, text_lengths = self.encoder(
            embedded_inputs, text_lengths)

        mel_output, states_travelled, input_parameters, output_parameters = self.hmm.sample(
            encoder_outputs)
    ...
    def sample(self, text_inputs, text_lengths):
        ...
        Args:
            text_inputs (int tensor) : vector of shape ([x]) where x is the phoneme input
        ...
        Returns:
            ...
            states_travelled (list): list of phoneme travelled at each time step t
                shape: (len)
        """
        text_inputs, text_lengths = text_inputs.unsqueeze(
            0), text_lengths.unsqueeze(0)
        embedded_inputs = self.embedding(text_inputs).transpose(1, 2)
        encoder_outputs, text_lengths = self.encoder(
            embedded_inputs, text_lengths)
