...
from src.utilities.plotting import (plot_alpha_scaled_to_numpy,
...
def log_validation(logger, model, mel_output, mel_output_normalised, state_travelled, mel_targets,
    ...
    logger.add_image(
        ...
        plot_alpha_scaled_to_numpy(
            model.hmm.log_alpha_scaled[0, :, :].T, plot_logrithmic=True),
    ...
    logger.add_image(
        ...
        plot_alpha_scaled_to_numpy(
            torch.exp(model.hmm.log_alpha_scaled[0, :, :]).T),
        iteration, dataformats='HWC')
    ...
    logger.add_image(
        ...
        plot_hidden_states_to_numpy(state_travelled), iteration, dataformats='HWC'
    )

    logger.add_image(
        "parameters/go_tokens", plot_go_tokens_to_numpy(
            model.hmm.go_tokens.clone().detach()), iteration,
        dataformats='HWC')

    states = [p[1] for p in input_parameters]
    transition_probability_synthesising = [
        p[2].cpu().numpy() for p in output_parameters]

    for i in range((len(transition_probability_synthesising) // 200) + 1):
        start = i * 200
        end = (i + 1) * 200
        logger.add_image(
            "synthesised_transition_probabilities/{}".format(i),
            plot_transition_probabilities_to_numpy(
    ...
    max_state_numbers = max_state_numbers.unsqueeze(
        1).unsqueeze(1).expand(max_len, 1, n_mel_channels)
